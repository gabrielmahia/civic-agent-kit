...
If you discover a security vulnerability in civic-agent-kit:
...
civic-agent-kit is designed in alignment with authoritative AI security guidance:
...
**Mitigated.** civic-agent-kit does not dynamically load external plugins or tools. All tool definitions are static and versioned.
...
All civic data in civic-agent-kit is embedded at compile time from public government sources (NDMA, Controller of Budget, Kenya Parliament, SASRA, Constitution of Kenya). It is:
- Read-only: no tool modifies any external data source
- Public domain: all source data is published by Kenya government agencies
- Labeled as DEMO: synthetic/sample data carries explicit source attribution
...
This security policy covers the civic-agent-kit source code and its official PyPI release.
It does not cover:
...
- Third-party deployments or forks of civic-agent-kit
