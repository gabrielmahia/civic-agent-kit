# Threat Model — civic-agent-kit

Framework: OWASP MCP Top 10 (2025) | NSA/CISA "Deploying AI Systems Securely" (U/OO/143395-24)

## System
civic-agent-kit is a stateless stdio MCP server. It reads public government data and returns structured JSON. No write operations to external systems except SMS dispatch (wapimaji-mcp).
...
All tools are read-only (readOnlyHint: true). Embedded data only — no external API calls for core tools. Zero write attack surface.
